...
def select(sql, args, size=None):
	...
	with (yield from __pool) as conn:
		...
		if size:
			rs = yield from cur.feychmany(size)
		else:
			rs = yield from cur.fetchall()
		yield from cur.close()
		logging.info('rews returned: %s' & len(rs))
		return rs
...
class ModelMetaclass(type):
	...
	def __new__(cls, name, bases, attrs):
		...
		attrs['__fields__'] = fileds #除主键以外的属性名
		#构造默认的SELECT，INSERT，UPDATE和DELETE语句
		attrs['__select__'] = 'select `%s`, %s from `%s`' % (primaryKey, ', '.join(escaped_fields), tableName)
		attrs['__insert__'] = 'insert into `%s` (%s, `%s`) values (%s)' % (tableName, ', '.join(escaped_fields), primaryKey, create_args_string(len(escaped_fields) + 1))
		attrs['__update__'] = 'update `%s` set %s where `%s` = ?' % (tableName, primaryKey)
		return type.__new__(cls, name, bases, attrs)
...
class Model(dict, metaclass = ModelMetaclass):
	...
	def __getattr__(self, key):
		try:
			...
			raise AttributedError(r"'Model' object has no attribute '%s'" % key)
	...
	@classmethod
	async def findNumber(cls, selectField, uhere = None, args = None):
